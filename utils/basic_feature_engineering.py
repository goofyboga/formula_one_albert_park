...
def compute_brake_balance(df):
    ...
    df["brake_front_avg"] = (df["M_BRAKESTEMPERATURE_FL_1"] + df["M_BRAKESTEMPERATURE_FR_1"]) / 2
    df["brake_rear_avg"] = (df["M_BRAKESTEMPERATURE_RL_1"] + df["M_BRAKESTEMPERATURE_RR_1"]) / 2
    df["brake_front_rear_diff"] = df["brake_front_avg"] - df["brake_rear_avg"]

    # Left vs right average
    df["brake_left_avg"] = (df["M_BRAKESTEMPERATURE_FL_1"] + df["M_BRAKESTEMPERATURE_RL_1"]) / 2
    df["brake_right_avg"] = (df["M_BRAKESTEMPERATURE_FR_1"] + df["M_BRAKESTEMPERATURE_RR_1"]) / 2
    df["brake_left_right_diff"] = df["brake_left_avg"] - df["brake_right_avg"]

    df.drop(columns=["brake_front_avg", "brake_rear_avg", "brake_left_avg", "brake_right_avg"], inplace=True)
...
def compute_tyre_balance(df):
    ...
    df.drop(columns=["tyre_front_avg", "tyre_rear_avg", "tyre_left_avg", "tyre_right_avg"], inplace=True)
